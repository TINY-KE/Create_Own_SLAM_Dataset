...
//设置一些保存路径
// string Path = "/home/robotlab/dataset/MySimDataset/gazebo_dataset_temp/";
// string pathRGB = Path + "rgb.txt";
// string pathDepth = Path + "depth.txt";
// string dataRGB = Path + "/rgb/";
// string dataDepth = Path + "/depth/";

string Path, pathRGB, pathDepth, dataRGB, dataDepth;
    ...
    try
    {
       //保存图像及路径
        ...
        if(isRGB)
            ...
            cv::cvtColor(cv_ptr->image, bgrImage, cv::COLOR_RGBA2RGB);
        else
...
void GrabDepth(const sensor_msgs::ImageConstPtr& msg)
{
    ...
    try
    {
        ...
        if (msg->encoding == sensor_msgs::image_encodings::TYPE_16UC1) {
            ...
            // 确定深度范围（单位通常是米）
            // float min_depth = 0.2;  // 最小深度值（米）
            // float max_depth = 15.0; // 最大深度值（米）
            
            // 将32FC1转换为16UC1（毫米单位）
            double scale = 5000;
            cv::Mat enlargedDepthImage = cv_ptr->image * scale;

            // 将深度图像的浮点数值转换为uint16
            cv::Mat uint16DepthImage;
            enlargedDepthImage.convertTo(uint16DepthImage, CV_16U);

            std::ostringstream osf;
            osf<< dataDepth <<std::fixed  <<time << ".png";

            // 保存为 TIFF 格式（保持浮点精度）
            cv::imwrite(osf.str(), uint16DepthImage);
...
int main(int argc,char **argv)
{
    ...
    if(argc < 2)
    {
        cout<<"please set the path to save the dataset"<<endl;
        return 1;
    }
    ...
    ros::start();
    ofstream of;
    of.open(pathRGB, std::ios_base::out | std::ios_base::trunc);
    if(of.fail()){
       ROS_INFO("Fail to opencv file!!");
    }else{
    ...
    of.open(pathDepth, std::ios_base::out | std::ios_base::trunc);
    if(of.fail()){
       ROS_INFO("Fail to opencv file!!");
    }else{
       of<<"#------------start a new dataset-----------------"; 
